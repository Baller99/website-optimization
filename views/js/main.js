...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
    ...
    var randomPizzas = document.getElementsByClassName("randomPizzaContainer")//Changed call to getElementsByClassName
...
var pizzasDiv = document.getElementById("randomPizzas");//Moved pizzasDiv outside of for loop
for (var i = 2; i < 100; i++) {
...
function updatePositions() {
  ...
  var items = document.getElementsByClassName('mover'); // changed querySelectorAll
  var len = items.length; //declared new variable len to hold items.length
  var constantArray = []; 
  var top = document.body.scrollTop; // moved outside the "for" loop and assigned it to a variable top to prevent creating the object each time.
  var phase;
  //Used constantArray to place repeating 5 phase values
  for (i = 0; i < 5; i++) {
      constantArray.push(Math.sin((top / 1250) + i));
  }
  //Pull phase value from constantArray
  for (i = 0, len = 40; i <len; i++) { 
        phase = constantArray[i % 5];
...
var elem = document.createElement('img');//Moved elem variable outside of loop
var movingPizzas = document.getElementById("movingPizzas1")//set new local variable and moved document.getElementById("movingPizzas1")
